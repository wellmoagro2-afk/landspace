"use client";

import dynamic from "next/dynamic";

// Lazy-load do componente de mapa apenas quando necessário
const LazyMapEmbed = dynamic(() => import("@/components/strategy/LazyMapEmbed"), {
  ssr: false,
  loading: () => (
    <div className="relative w-full aspect-video rounded-sm overflow-hidden border border-[rgba(0,0,0,0.12)]">
      <div className="absolute inset-0 flex items-center justify-center bg-gradient-to-br from-[rgba(0,0,0,0.02)] to-[rgba(0,0,0,0.05)]">
        <div className="text-center space-y-3">
          <div className="w-16 h-16 mx-auto bg-[rgba(0,184,107,0.1)] rounded-full flex items-center justify-center border border-[rgba(0,184,107,0.2)]">
            <svg
              className="w-8 h-8 text-[#00B86B] animate-pulse"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
                strokeWidth={2}
                d="M9 20l-5.447-2.724A1 1 0 013 16.382V5.618a1 1 0 011.447-.894L9 7m0 13l6-3m-6 3V7m6 10l4.553 2.276A1 1 0 0021 18.382V7.618a1 1 0 00-.553-.894L15 4m0 13V4m0 0L9 7"
              />
            </svg>
          </div>
          <p className="text-sm font-sans text-[#6A6A6A]">Carregando mapa...</p>
        </div>
      </div>
    </div>
  ),
});

interface MapEmbedClientProps {
  mapEmbedUrl: string;
  title?: string;
}

export default function MapEmbedClient({ mapEmbedUrl, title = "Mapa Interativo" }: MapEmbedClientProps) {
  return <LazyMapEmbed src={mapEmbedUrl} title={title} />;
}
